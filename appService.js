...
const dbConfig = {
    ...
    password: envVariable.ORACLE_PASS,
...
async function selectTable(selectedTable, projection, filter) {
    return await withOracleDB(async (connection) => {
        // var sql = `SELECT `.concat(selectedTable, `, `,
        //     `FROM `, )
        const result = await connection.execute(
            `SELECT :projections FROM :selectedTable`,
                [selectedTable, projections, filter],
                { autoCommit: true }
        );
        return result.rows;
    }).catch(() => {
